...
class RegisterViewModel @Inject constructor(
    ...
    fun registerUser(userRegistration: DataModel.Data) {
        loading.value = true
...
/*  fun CreateUsers(userData: DataModel) {
        viewModelScope.launch {
            _user.value = Resource.loading()
            val result = postUserUseCase()

            if (result.isSuccess()) {
                result.data?.let {
                }
                _user.value = Resource.success(userData)
            } else {
                _user.value = Resource.error(message = result.message)

            }
        }
         viewModelScope.launch {
                _registrationResult.value = shoppingRespository.createUser(userRegistration)
